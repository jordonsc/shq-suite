...
|-----------|----------|-------------|
| `nyx/` | Rust | **⚠ DEAD — the estate does not run this.** Display server for kiosks (brightness, auto-dim, CDP navigation, clock-screensaver control). Superseded by the Argus repo's **`kiosk`** service (`jordonsc/argus`, `kiosk/`), which absorbed nyx **and** chronos into one binary at kiosk 1.0.0 and deleted both; the fleet has run `kiosk` since 2026-08-01 (unit `kiosk.service`, "Argus Kiosk (session owner)"). Build/deploy from the Argus repo. Kept here for history only — an agent sent to "the display agent, nyx" will be reading code nothing executes. |
| `chronos/` | Rust | **⚠ DEAD — see `nyx/`.** Fullscreen clock overlay (wlr-layer-shell) shown over the kiosk dashboard as an optional idle screensaver. Ported in-process into the Argus repo's `kiosk` binary (`kiosk/src/clock.rs`) and deleted there; no longer spawned as a child process. |
