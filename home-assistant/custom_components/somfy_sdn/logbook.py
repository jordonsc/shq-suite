"""Logbook rendering for `somfy_sdn_diag` events.

Twin of actron_mitm_controller/logbook.py — keep the two in step.
...
def async_describe_events(hass: HomeAssistant, async_describe_event) -> None:
    ...
    def async_describe_diag(event: Event) -> dict[str, str]:
        ...
        return {
            # MAC first: it is the identity that survives a DHCP change, which is the whole
            # reason this integration is MAC-keyed. Host only when the MAC isn't known yet.
            LOGBOOK_ENTRY_NAME: f"Somfy SDN ({data.get('mac') or data.get('host', 'controller')})",
