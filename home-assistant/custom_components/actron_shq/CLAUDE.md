...
- **Climate** (`climate.py`): Main + zone entities. Temperature/fan commands go through `_run_command()` which sets the coordinator overlay, acquires `command_lock`, and runs the API call once. Per-slot cancellation (e.g. "temperature", "mode") discards in-flight predecessors so rapid slider drags only send the final value. Optimistic state lives on the coordinator, not the entity, so concurrent edits across entities don't clobber each other.
- **Main-unit HVAC mode** (`climate.py` → `async_set_hvac_mode` / `_send_mode_until_confirmed`): like zone on/off, the cloud often accepts a mode change (HTTP 200) without applying it, so the command is **re-sent** every `MODE_RESEND_INTERVAL_SECONDS` until the `is_on`/`mode` overlay clears (controller confirms or reconciles) or `MODE_CONFIRM_TIMEOUT_SECONDS` elapses; it bails immediately on rate-limit and calls `extend_burst()` each tick. Uses the "mode" slot so a newer mode request cancels an in-flight resend loop.
...
| File | Purpose |
|------|---------|
| `const.py` | Domain, poll intervals (base 30s, burst 1s for 15s, rate-limit cooldown 60s), zone retry (resend 5s / timeout 90s), mode retry (resend 5s / timeout 90s) and last-zone-off hold (60s) |
