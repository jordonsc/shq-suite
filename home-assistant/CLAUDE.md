...
**Key files**: `client.py` (+ diag/health/backlog dispatch, close-code capture), `coordinator.py` (`controller_key`, `_mac_norm`/`_format_mac`, diag bus events, health store), `config_flow.py` (`VERSION`), `__init__.py` (`async_migrate_entry`), `entity.py` (shared bases + controller `device_info`), `cover.py` (per-motor entity + entity services), `sensor.py` (bus + firmware diagnostics), `logbook.py`, `services.yaml`, `const.py`.
