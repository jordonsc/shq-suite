...
| `actron_mitm_controller` | WebSocket | 8767 | Config Flow | Actron A/C via local MITM bridge (actron-sniffer ESP32) + firmware diagnostics (sensors + `actron_mitm_diag` events) |
...
**Reconnect backoff (`actron_mitm_controller` 1.9.0 / `somfy_sdn` 1.13.0, ledger shq-suite-0046):** `RECONNECT_DELAY_MIN_S = 2` after any session that delivered at least one frame, doubling towards `RECONNECT_DELAY_MAX_S = 30` only while attempts keep failing (a refused connect, or a socket that died before its first frame — the one case where hammering a device would fill its five WS slots). The old flat 30 s sat *above* the 30 s availability timeout, so every session death was a guaranteed 16 s+ entity outage (754 s/day on the worst somfy controller). `_last_msg_time` is now stamped on connect, so a fresh socket gets the full 30 s to deliver its snapshot instead of being judged on the previous session's last message; `AVAILABILITY_TIMEOUT_S` itself is unchanged — nothing is masked. `_connecting` flag in the coordinator prevents parallel connect tasks (the availability monitor and a `_reconnect_after`-driven connect could otherwise race during the 5 s connect window).
...
**Close-code logging (1.1.2 at WARNING; dialled to INFO in 1.1.3 once the fix was confirmed, 2026-07-19; 1.9.0 logs WHO closed).** `client.run()` logs the WS close code + socket lifetime on every disconnect (was a bland INFO "closed by peer"). Since 1.9.0 it reads the legacy exception's `rcvd`/`sent` close frames rather than the ambiguous `.code` (1006 whenever no close frame crossed in *either* direction — which is what our own pong timeout produces) and the `ha_closed` event carries `close_rcvd` / `close_sent` / `closed_by` (`device` / `ha` / `nobody`) + `frames` received, so HA's own record says who hung up. Added to diagnose the recurring ~30 s `unavailable` flaps, which were root-caused to **firmware loop-starvation** (the actron-sniffer RS485 bridge task starving the ESP's HTTP+WS loop so it went silent, tripping the 30 s availability timeout) — fixed by a yield budget in `actron-sniffer/src/main.cpp`, verified 0 flaps over 10 h under heating load; see ledger shq-suite-0019. The flaps came through the coordinator availability path, so this branch stays quiet post-fix; it fires when a command hits an already-silent socket (the "no close frame received or sent" toast) or on a genuine close. Kept at INFO as a cheap regression signal — bump back to WARNING if you need it in `/api/error_log`. **⚠️ The flaps regressed 2026-07-25 with a new device-side signature (silent socket death, not starvation) — see `actron-sniffer/CLAUDE.md` "Regression #2" + ledger shq-suite-0019; instrumented firmware flashed 2026-08-03, verdict due ~08-08.**

**Diagnostics (1.2.0, 2026-08-23 — ledger shq-suite-0038).** The firmware now says *why* it dropped a socket instead of leaving it to be inferred from the far end; this component receives that and puts it in HA's history. See `actron-sniffer/CLAUDE.md` → "Self-diagnostics" for the device side.

- **`sensor.actron_ac_fault` + `binary_sensor.actron_ac_problem`** (component 1.6.0, ledger shq-suite-0041). The fault sensor's state is a short slug (`ok`, `clock_stalled`, `bus_offline`, `ws_capacity`, `heap_low`; `clock_rebase` / `clock_word_step` were retired in fw 1.14.3 — a handled clock event is logged via the `clock_glitch` record and the clock sensors, never latched as a Problem, ledger shq-suite-0050) with the human sentence and the full active bitmask as attributes; the binary sensor is the same signal as a `PROBLEM` boolean for automations. Deliberately a *text* sensor: a boolean cannot say which fault, and "clock stalled" and "heap low" want very different responses.
- **`button.actron_ac_reboot`** (component 1.6.0) — sends the `reboot` WS command. Manual only; nothing auto-reboots on a fault, because a reboot destroys the RAM-only diagnostic ring that makes the fault diagnosable.
- **WiFi MAC transmit sensors** (component 1.7.0, fw 1.12.0 — twin of the `somfy_sdn` 1.11.0 set, see below): `sensor.actron_ac_wifi_frames_acknowledged` / `_wifi_tx_retries` / `_wifi_tb_retries` / `_wifi_ack_timeouts` / `_wifi_collisions` / `_wifi_tx_buffer_starvation` / `_wifi_tx_failures` (totals) + `_wifi_phy_mode` (text). Read `null` until the bridge is flashed with fw ≥ 1.12.0.
- **`select.actron_ac_wifi_protocol` + `sensor.actron_ac_wifi_protocol`** (component 1.8.0, fw 1.13.0 — twin of the `somfy_sdn` pair, see below). The select (config category, `bgnax`/`bgn`/`bg`) sends `set_wifi_proto{proto}`; the firmware persists it and — **since fw 1.14.0 / component 1.9.0 — REBOOTS to apply it** (the protocol bitmap is honoured only on a fresh WiFi init; the 1.13.0 re-associate did not apply it), so this now carries the same A/C cost as Reboot: **zones off first**. The diagnostic sensor mirrors the health push's `proto`. The bridge is the *control* in the A/B — leave it on `bgnax`.
- **Three-frame health push + `ws_evict` (component 1.9.0, fw 1.14.0, ledger shq-suite-0046).** fw ≥ 1.14.0 sends the vitals as `health` / `health_ws` / `health_net` a second apart (each under the firmware's 600 B frame budget) and the diag backlog a record per `diag` frame instead of one `diag_backlog`; `coordinator._on_health` merges the frames into one dict so every sensor reads as before, and `diag_backlog` is still accepted from older firmware. New logbook sentence for `ws_evict` (the firmware's liveness policy dropped a client after 45 s with nothing inbound) and a `stall_reap` disconnect reason; `pong_timeouts` now counts those evictions (older fw: a pong 25 s late).
- **21 diagnostic sensors** (`sensor.py`, all `EntityCategory.DIAGNOSTIC`) fed by the firmware's 30 s `health` push: free/min heap, largest free block, **spare sockets**, WS clients/connects/disconnects, **pong timeouts / peer closes / transport errors**, **worst loop stall**, **worst HTTP stall**, loop stalls, WiFi disconnects, heartbeat age, RSSI, uptime, **clock backward reads / high-word faults / re-baselines** (1.6.0 — read the backward-read count as a RATE; thousands per second means the device's clock is pinned right now, which is what a nine-hour somfy outage looked like), plus `sensor.actron_ac_last_disconnect_reason` whose attributes carry the full evidence for that disconnect (socket lifetime, pong age, traffic counts, heap/sockets/loop-stall at that instant). They exist so the vitals land in the recorder and can be plotted against the flaps — the previous investigation had to sample `/stats` over HTTP by hand, which **perturbed the fault it was measuring** (15 s polling quadrupled the flap rate).
- **Bus event `actron_mitm_diag`** for every firmware record *and* for our own end of the connection (`ha_connected`, `ha_connect_failed`, `ha_clean`, `ha_closed`, `ha_error` with close code + socket lifetime) — so both ends can be read against each other, which is what settles who hung up on whom. `logbook.py` renders each as a sentence. Notable events log at WARNING (visible in `/api/error_log`), the rest at INFO.
- **De-duplication on `seq`** (`DIAG_SEEN_MAX = 128`): the firmware replays a backlog on every connect — the only way we ever learn why the *previous* session ended, since a socket cannot be told about its own death — so the same record legitimately arrives twice.
- **Availability now keys off any inbound frame**, not just `state` (`client.set_message_callback`). A `health` or `diag` frame proves the socket is alive; previously a device talking to us perfectly well could still be marked unavailable if the state push specifically stalled.
- Note `unclassified`, not `unknown`, for a disconnect the firmware could not attribute — HA renders the literal string `unknown` as the no-data state, which would make "no evidence" look identical to "never heard from it".
...
**Key files**: `client.py` (WebSocket + ack correlation + keepalive + close-code logging + diag/health/backlog dispatch), `coordinator.py` (connection + dispatch + availability + reconnect + diag bus events), `config_flow.py` (IP+port form), `climate.py` (master + zone entities), `sensor.py` (diagnostics), `select.py` (WiFi protocol A/B knob), `logbook.py` (event rendering), `const.py` (timeouts + diag constants).
...
**Entities** (platforms: cover, button, switch, number, select, sensor, binary_sensor — shared base in `entity.py`). A **controller device** per ESP32 (buttons: Rediscover motors, Reboot, Reconnect WiFi (`reconnect_wifi` admin command — re-scan + reassociate to the strongest AP, for moving off a distant AP the device fell back to at boot); switch: Bus active = ACTIVE/LISTEN; diagnostic sensors: motors-online, wire-errors, Fault; binary_sensor: Problem) and a **per-motor device** alongside each cover holding `EntityCategory.CONFIG`/`DIAGNOSTIC` calibration entities (switch: Reversed; binary_sensor: Fault; numbers: Jog duration + Bottom limit (pulses, stateful read/write); buttons: Set top/bottom limit, Identify, Reset positions, Jog up/down — Jog fires a timed CTRL_MOVE nudge that works before limits). Devices are deletable from the UI (`async_remove_config_entry_device` forgets them on the controller). Config-category = they live on the device page, not dashboards. New motors auto-create their device when they appear in state (after a rediscover).
...
**Diagnostics (1.5.0, 2026-08-23 — ledger shq-suite-0038).** Ported from `actron_mitm_controller` 1.2.0 alongside firmware 1.6.0; see `somfy-sdn/CLAUDE.md` → "Self-diagnostics" for the device side. This fleet is where it matters most — Bed 4 was caught in a locked 40 s `unavailable` flap with the heartbeat firing normally and nothing on the device able to say why.

- **`sensor.<controller>_fault` + `binary_sensor.<controller>_problem`** (component 1.9.0, ledger shq-suite-0041), on the *controller* device. Slug state (`ok`, `clock_stalled`, `bus_offline`, `ws_capacity`, `heap_low`; `clock_rebase` / `clock_word_step` were retired in fw 1.14.3 — a handled clock event is logged via the `clock_glitch` record and the clock sensors, never latched as a Problem, ledger shq-suite-0050) with detail + active bitmask as attributes, plus a `PROBLEM` boolean for automations. **This is the gap that let Bed 2 sit wedged for nine hours:** the only fault entity was the per-motor `binary_sensor.<motor>_fault`, which reports the SDN motor's own status word and knows nothing about the controller hosting it — the motor was reporting `ok` throughout.
- **`button.<controller>_reboot`** (component 1.9.0) — sends the `reboot` WS command. Manual; nothing reboots on a clock fault alone, because a reboot destroys the RAM-only diagnostic ring. Bed 2 was root-caused precisely *because* nobody rebooted it. The firmware's own self-heal (fw 1.11.0) re-associates the WiFi link first and reboots only if that did not help — see below.
- **`sensor.<controller>_gateway_probe_failures` + `sensor.<controller>_network_recoveries`** (component 1.10.0, fw 1.11.0, ledger shq-suite-0044). The firmware pings its gateway once a minute; probe failures climbing while the device is still associated is the "reachable by the router, unreachable by everyone" signature that killed Bed 2 on 2026-09-03 after a backward clock step blacked out the WiFi driver's timers (heap leaked 237 k -> 8 k over 18 h). Recoveries counts the re-associations the firmware performed on its own (backward clock step >= 60 s, gateway unanswered 3 min with nothing inbound over WS, or heap < 60 kB for 10 min). The manual cure for that signature is a UniFi client reconnect, not a power-cycle, and it keeps the evidence.
- **WiFi MAC transmit sensors** (component 1.11.0, fw 1.12.0, ledger pending — the long-frame uplink-loss investigation): `sensor.<controller>_wifi_frames_acknowledged` / `_wifi_tx_retries` / `_wifi_tb_retries` / `_wifi_ack_timeouts` / `_wifi_collisions` / `_wifi_tx_buffer_starvation` / `_wifi_tx_failures` (all `TOTAL_INCREASING`, from the health push's `tx_ok`/`tx_retry`/`tx_tbretry`/`tx_to`/`tx_coll`/`tx_nomem`/`tx_fail`) + `_wifi_phy_mode` (text, `"HE20 ch6 bw20 bgnax"`). Plot retries and ACK timeouts *per acknowledged frame* against the churning pair vs a clean neighbour. The logbook's disconnect / stall-reap lines now end with the MAC deltas since the previous record and lwIP's view of the socket (`tcp_state`, `tcp_nrtx`, `tcp_rto_ms`, `tcp_unacked`, `tcp_cwnd`); `ws_stall_reap` records finally render as a sentence instead of a raw dict. All `null` until a controller runs fw ≥ 1.12.0. Field meanings: `somfy-sdn/CLAUDE.md` → "MAC transmit telemetry".
- **`select.<controller>_wifi_protocol` + `sensor.<controller>_wifi_protocol`** (component 1.12.0, fw 1.13.0, ledger shq-suite-0046 — the A/B instrument for the long-frame uplink loss). The select (`select.py`, config category, options `bgnax`/`bgn`/`bg`) sends `set_wifi_proto{proto}`: the firmware persists it to NVS, acks, then — **since fw 1.14.0 / component 1.13.0 — REBOOTS to apply it** (`esp_wifi_set_protocol()` is honoured only on a fresh WiFi init: the 1.13.0 re-associate left the canary on HE20 with `bgn` persisted, a reboot came up HT20), so the controller is gone ~10 s and its RAM diagnostic ring is lost; its current option is read from the health push's `proto`, so it shows `unknown` until the first push after (re)connect.
- **Robustness build (component 1.13.0, fw 1.14.0, ledger shq-suite-0046).** HA side: reconnect backoff 2 s → 30 s (see the Actron reconnect note — `RECONNECT_DELAY_MIN_S`/`_MAX_S`), `_last_msg_time` stamped on connect, `ha_closed` carries `close_rcvd`/`close_sent`/`closed_by`/`frames`, and the three-frame `health`/`health_ws`/`health_net` push is merged in `coordinator._on_health` (older firmware's single `health` and `diag_backlog` still work). Logbook renders `ws_evict` and the `stall_reap` reason. Device side: `somfy-sdn/CLAUDE.md` → "WS liveness". `bgnax` is the untouched default. The diagnostic text sensor records the same `proto` so churn history can be split by setting — read it against `_wifi_phy_mode`, which is what the station actually negotiated (`HE20` = still on 11ax). **Flip only the unit under test, never the control** — the run-book is in `somfy-sdn/CLAUDE.md` → "WiFi protocol A/B". There is deliberately no HA button for `POST /phycal` (it reboots and is a one-shot remedy, not a control).
- **1.13.0 also fixes the `tcp_*` fields** that never arrived from any 1.12.x unit (a dual-stack sockaddr the firmware rejected — `somfy-sdn/CLAUDE.md` → "MAC transmit telemetry", 1.13.0 corrections), adds `tcp_miss` to the health push, and drops `tx_rtt_max_us`, which was never microseconds. No HA sensor depended on either.
- **22 diagnostic sensors per controller** (`sensor.py`, `EntityCategory.DIAGNOSTIC`, on the *controller* device) fed by the firmware's 30 s `health` push: free/min heap, largest free block, **spare sockets**, WS clients/connects/disconnects, **pong timeouts / peer closes / transport errors**, **worst loop stall**, **worst HTTP stall**, loop stalls, WiFi disconnects, heartbeat age, RSSI, uptime, **clock backward reads / high-word faults / re-baselines** (1.9.0 — read the backward-read count as a RATE, not a total: the healthy floor is a few hundred over four days, and thousands per second is a clock pinned right now), plus `..._last_disconnect_reason` carrying the full evidence for that disconnect as attributes.
- **These sensors deliberately override `SomfySdnControllerEntity.available` to always be `True`.** Their whole purpose is to describe the run-up to a dropout; going unavailable with the link would discard the evidence at exactly the moment it becomes interesting.
- **Bus event `somfy_sdn_diag`** for every firmware record *and* for our own end (`ha_connected`, `ha_connect_failed`, `ha_clean`, `ha_closed`, `ha_error` with close code + socket lifetime). `logbook.py` (twin of the actron one) renders each as a sentence, named by MAC since that is the identity that survives a DHCP change. Notable events log at WARNING.
- **De-duplication on `seq`** (`DIAG_SEEN_MAX = 128`): the firmware replays a backlog on every connect — the only way we learn why the *previous* session ended — so records legitimately arrive twice.
- **Availability keys off any inbound frame**, not just `state` (`client.set_message_callback`).
