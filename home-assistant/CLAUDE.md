...
- **17 diagnostic sensors per controller** (`sensor.py`, `EntityCategory.DIAGNOSTIC`, on the *controller* device) fed by the firmware's 30 s `health` push: free/min heap, largest free block, **spare sockets**, WS clients/connects/disconnects, **pong timeouts / peer closes / transport errors**, **worst loop stall**, **worst HTTP stall**, loop stalls, WiFi disconnects, heartbeat age, RSSI, uptime, plus `..._last_disconnect_reason` carrying the full evidence for that disconnect as attributes. 240 entities across the 12 controllers.
