...
- **`sensor.actron_ac_fault` + `binary_sensor.actron_ac_problem`** (component 1.6.0, ledger shq-suite-0041). The fault sensor's state is a short slug (`ok`, `clock_stalled`, `bus_offline`, `ws_capacity`, `clock_rebase`, `clock_word_step`, `heap_low`) with the human sentence and the full active bitmask as attributes; the binary sensor is the same signal as a `PROBLEM` boolean for automations. Deliberately a *text* sensor: a boolean cannot say which fault, and "clock stalled" and "heap low" want very different responses.
...
- **`sensor.<controller>_fault` + `binary_sensor.<controller>_problem`** (component 1.9.0, ledger shq-suite-0041), on the *controller* device. Slug state (`ok`, `clock_stalled`, `bus_offline`, `ws_capacity`, `clock_rebase`, `clock_word_step`, `heap_low`) with detail + active bitmask as attributes, plus a `PROBLEM` boolean for automations. **This is the gap that let Bed 2 sit wedged for nine hours:** the only fault entity was the per-motor `binary_sensor.<motor>_fault`, which reports the SDN motor's own status word and knows nothing about the controller hosting it — the motor was reporting `ok` throughout.
