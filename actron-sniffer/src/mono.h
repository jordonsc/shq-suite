...
// bad, and re-baseline onto it. On this firmware every now() call is on the Arduino main loop —
// bridgeTask deliberately uses raw millis() in its hot yield path — so the budget clears in a few
// seconds rather than the somfy's sub-second. It must stay far above any run of backward reads a
// race can produce: those are ISOLATED samples, and any forward read resets the run to zero.
