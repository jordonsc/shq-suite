...
**Fleet state (2026-09-06 01:38 UTC):** the in-wall bridge runs **fw 1.14.3** (HA `actron_mitm_controller`
1.9.0), flashed over the air after the whole somfy fleet (12 units) had taken 1.14.3 under the canary rule
and with every `climate.actron_ac_*` off, back with fault `ok` in 10 s. 1.14.2 (2026-09-05) was the first
1.12+ build on the bridge hardware — HE20, `tx_en=15`; 1.14.3 retires the `clock_rebase` / `clock_word_step`
faults and clamps the loop meter across a re-baseline (shq-suite-0049/0050). The A/C rule still applies to every future
...
| `src/fault.h` / `src/fault.cpp` | Pure C++ device-level fault registry (twin of `somfy-sdn/src/fault.{h,cpp}`). Severity-ordered bitmask (`clock_stalled`, `bus_offline`, `ws_capacity`, `heap_low`; the `clock_rebase` / `clock_word_step` codes were removed in fw 1.14.3 — a handled clock event is a counter and a `clock_glitch` record, not a latched Problem, ledger shq-suite-0050) + one-line detail, surfaced in `/stats` (`fault=`/`fault_detail=`), the WS health push, and HA's `sensor.actron_ac_fault` + `binary_sensor.actron_ac_problem`. The evaluator lives in `main.cpp` (`updateFaults`). |
