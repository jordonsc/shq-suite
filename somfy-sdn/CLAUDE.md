...
design rationale: [`SPEC.md`](SPEC.md). **fw 1.14.3 is live on all 12 controllers (2026-09-06 01:23-01:37 UTC,
canary rule: Bed 2 first, 3 min clean, then eleven gated pulls at 45 s, zero retries; HA `somfy_sdn`
1.13.0)** and on the actron twin's bridge (01:38, zones off). 1.14.3 retires the `clock_rebase` /
`clock_word_step` faults and clamps the loop meter across a re-baseline (shq-suite-0049/0050); 1.14.2's
first night measured the WS churn gone (two residual HA-timer drops in 18 h, shq-suite-0046). The generations it carries, oldest first:
...
`clk_jumpms` in `/stats`, and as HA sensors. **They are counters and a `clock_glitch` diag record,
not faults (fw 1.14.3, ledger shq-suite-0050).** fw 1.10.0 latched `clock_rebase` /
`clock_word_step` until reboot; Bed 4 then sat with its HA Problem sensor on for a 3.3 s step the
filter had handled perfectly. A fault is for a condition that needs a response — a handled event
is logged, and nothing more. `mono::elapsed(from, to)` is the clamped duration between two reads
in one pass: a re-baseline between them made the loop meter record 4,294,963,989 ms
(shq-suite-0049); use it for any measured duration, and the unsigned `(now - last) >= interval`
form for deadlines. **Read `clk_back` as a RATE**: a healthy controller
