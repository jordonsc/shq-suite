...
enum class Event : uint8_t {
  ...
  NetRecover,        // the network-stack watchdog acted (fw 1.11.0, shq-suite-0044); `ip` =
                     // the reason ("clock-step", "unreachable", "heap-low", "stack-dead"),
                     // `value` = re-associations so far. A reboot reason lands in the next
                     // boot's note= as well, since the ring does not survive it.
...
// The network-stack watchdog is about to re-associate or reboot for `reason`; `recoveries` is
// its lifetime re-association count. Recorded BEFORE the action so the heap/rssi stamp shows
