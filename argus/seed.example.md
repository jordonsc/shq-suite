...
## Trigger-profile guidance (tiered response — two flows)

> Argus assesses with a *posture* set by WHY it was woken. The per-scenario message
> prompt frames the look; this seed supplies the premises-specific judgement. Fill
...
- **Investigate** (a presumed-benign approach, e.g. a person at the front door) —
  `<what an ordinary visitor / delivery looks like here: a resident arriving, a
  known delivery window, the cleaner's day/time, regular couriers, expected
  vehicles>` vs `<what genuine danger looks like: a visibly brandished weapon, a
  balaclava / concealed face, forced-entry behaviour, or a person under duress>`.
  Argus runs a silent, shallow double-check and promotes to a full alarm (and trips
  the real panel) ONLY on structured danger (a weapon, an injury, duress, or a
  `critical` read); otherwise it quietly stands down. `<any front-door-specific
  notes — intercom, regular couriers, etc.>`
