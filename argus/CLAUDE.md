...
| `src/out/kiosks.rs` | **Phase 4** kiosk-takeover consumer. Consumes BOTH the `watch<Option<CaseState>>` AND the `watch<AlarmMode>` channels and computes a `desired_target`: **HUD** (`<public_base>/alarm`) when a case is present (incl. the cleared/AUTHORISED dwell) OR the alarm is `Arming`/`Armed`/`Triggered`/`Authorised`; else the kiosk's **Dashboard**. Navigates (`shq_display.navigate`) only when the target changes — ONE `/alarm` URL covers arming→triggered→cleared (content switches over the WS, no re-navigation). **Phase 5 (0.30.0): drives nyx's screen wake/keep-awake.** Every HUD takeover navigate carries `wake: true` (wakes the backlight + kills any Chronos clock overlay BEFORE the navigate, so the pane shows on a sleeping/clock kiosk); `keep_awake` rides the pure `alarm_active(case, mode)` — `true` ONLY during an actual incident (a `Triggered` alarm or a live non-gated case), to PIN the HUD on so it can't blank mid-incident; `false` for arming/armed/authorised standby so an armed-away house doesn't keep kiosks lit 24/7. The kiosk needs no pre-trigger pin: the `Triggered` takeover navigate carries `wake:true`, which WAKES a kiosk that blanked while armed (0.34.4 — reverted the 0.34.3 arming-delay pin per the user; a wake-on-trigger is all that's needed). The restore-to-dashboard navigate sends `wake:false, keep_awake:false` (release the pin → normal idle/blank resumes). Requires nyx ≥ 1.2.0 + shq_display component ≥ 1.2.0 (older nyx ignores the fields). **LIVE-FIRED on kiosk11** (2026-06-19; pre-Phase-5). nyx clock-overlay caveat is now resolved by the wake field |
...
navigate, `keep_awake` = `alarm_active` (force-on only during an incident:
`Triggered` / a live case; arming/armed blank normally — the `Triggered`
takeover's `wake:true` wakes a blanked kiosk on trigger — 0.34.4),
`keep_awake:false` on the dashboard restore. Old nyx (< 1.2.0) ignores the
...
- **HUD radar sweep is opt-in (0.34.0)**: the rotating radar was too much. `hud.js` gates `startRadar()` behind `const RADAR_ENABLED = false` plus a `?radar=1` URL override; off = no RAF loop, `#radar` hidden. Static chrome (tick rail, crosshairs, emblem) stays. **HUD arming vs armed + threat urgency (0.34.0)**: `SYS.arming` now uses `state:"arming"` (amber `[data-state="arming"]` theme + warning pulse) so arming reads as caution vs armed's calm steel; `applyTheme` sets `data-threat` so `[data-threat="life_threatening"]` adds an extra urgency cue (amplified border pulse + full-frame vignette + headline throb) over the base red, calmer at `threat_present`.
