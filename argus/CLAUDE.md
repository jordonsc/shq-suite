...
> shape-only (no routing key). **Phase 4 takeover is NOT live-fired** (HUD server
> built + locally smoke-tested; no real kiosk navigated). The `web/` HUD app is
> owned by the frontend; the Rust side serves it. **A large post-M1 live-hardening
> pass (argus 0.7.0→0.19.0 + Overwatch 0.3.0 blocking verbalise) is captured in
> [`specs/argus/06-refinements.md`](../specs/argus/06-refinements.md) — read it
> first for the current state + the TEST-POSTURE the house is left in.** **Next M1
...
| `deploy-container.sh` | One-shot deploy: rsync a minimal context (argus/ minus `target/` + overwatch/proto) to atlas, `podman build` there, install the Quadlet unit, `daemon-reload` + `restart`. Refuses to start if `config.yaml`/`argus.env` are absent. Does **not** touch secrets. |
