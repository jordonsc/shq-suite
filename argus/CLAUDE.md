...
| `src/out/kiosks.rs` | **Phase 4** kiosk-takeover consumer. Consumes BOTH the `watch<Option<CaseState>>` AND the `watch<AlarmMode>` channels and computes a `desired_target`: **HUD** (`<public_base>/alarm`) when a case is present (incl. the cleared/AUTHORISED dwell) OR the alarm is `Arming`/`Armed`/`Triggered`/`Authorised`; else the kiosk's **Dashboard**. Navigates (`shq_display.navigate`) only when the target changes — ONE `/alarm` URL covers arming→triggered→cleared (content switches over the WS, no re-navigation). **Phase 5 (0.30.0): drives nyx's screen wake/keep-awake.** Every HUD takeover navigate carries `wake: true` (wakes the backlight + kills any Chronos clock overlay BEFORE the navigate, so the pane shows on a sleeping/clock kiosk); `keep_awake` rides the pure `alarm_active(case, mode)` — `true` during the `Arming` delay, a `Triggered` alarm, or a live non-gated case, to PIN the screen on so the HUD can't blank; `false` for STEADY `armed`/authorised standby so an armed-away house doesn't keep all kiosks lit 24/7 (0.34.3 — the user's choice; a trigger re-pins with `wake:true`, waking a kiosk that blanked while armed). The restore-to-dashboard navigate sends `wake:false, keep_awake:false` (release the pin → normal idle/blank resumes). Requires nyx ≥ 1.2.0 + shq_display component ≥ 1.2.0 (older nyx ignores the fields). **LIVE-FIRED on kiosk11** (2026-06-19; pre-Phase-5). nyx clock-overlay caveat is now resolved by the wake field |
...
navigate, `keep_awake` = `alarm_active` (force-on during the `Arming` delay /
`Triggered` / a live case; steady `armed` blanks normally — 0.34.3),
