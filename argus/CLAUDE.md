...
> **Trigger profiles — Phase 4a LANDED (0.28.0, scaffolding):** the
> `Alarm`/`Investigate`/`General` rails — `TriggerProfile` on `CaseState`
> (`trigger_profile` = how it opened, `effective_profile` = current posture),
> per-source `triggers:` config, and a DORMANT output/kiosk gate (`CaseState::gated()`)
> — all default to `Alarm`, so behaviour is byte-for-byte identical to 0.27.0 (every
> trigger today is the house alarm → `Alarm` → `Elevated`, never gated). **Phase 4b**
> (the escalation/promotion policy, softer-trigger firing, trip-real-alarm, and
> profile-parameterised prompts) is **pending** — see
...
- **Trigger-profile gate is DORMANT in 4a (0.28.0)**: `CaseState::gated()` (true when `effective_profile != Alarm`) gates outputs in TWO choke points — `out::route_event` (`if state.gated() && kind != Escalated { return }` — suppresses voice/klaxon/PD) and `kiosks::desired_target` (a gated case doesn't drive takeover; the arming/armed/triggered/authorised panes ride the alarm MODE and are unaffected). The HUD `/kiosk` WS + the case journal are NOT routed through `route_event`, so they're never gated (correct — the HUD always reflects the live assessment). **It is inert today**: every trigger is the house alarm → `Alarm` → never gated, so no case is ever gated and both guards are no-ops. Phase 4b wires the softer triggers + the promotion (an `Escalated` milestone flipping `effective_profile` to `Alarm`) that makes the gate live. Don't "simplify away" the gate as dead code — it's the safety rail for 4b.
