...
- **MAIN image is priority-ranked + NOT concern-gated (0.34.0)**: the live test produced a BLANK HUD because still-capture was gated behind `Person::is_subject_of_concern` and the subject read as a resident. Fix: the engine now ALWAYS captures a MAIN still each active tick (reusing the frames already grabbed — no extra HA snapshot), stored on `CaseState.main_still: Option<StillRef>` + `main_still_category` (`life_threatening`/`malicious`/`intruder`/`any`). It is PRIORITY-RANKED and STICKY-DOWNWARD: rank 3 life_threatening / 2 malicious / 1 person-of-concern / 0 any-person-present; a newer still replaces the held one only when `tick_rank ≥ main_still_rank` (`ActiveCase.main_still_rank`, reset per case) — so a higher category holds over later ambiguous frames, and an ambiguous frame never displaces an intruder one. Pure `compute_main_rank`/`should_replace_main` (unit-tested). The HUD (`hud.js render()`) renders `caseState.main_still` as the primary pane (falls back to `pickPrimaryView` when absent), so the pane is never blank with activity. **Intruder ID mugshot cards stay concern-gated** — only the MAIN image is broad; `upgrade_persons`/`subjectsOfConcern` are unchanged (don't card a resident/guest).
